...
public class SyncLocationClass {

    private Context mContext;
    private int mtype;

    public SyncLocationClass(Context mContext, int mtype) {
    ...
    public void syncOnBackGround() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
              ...
              */
                Integer lastsyncvalue=GetLocationSensorsDataAsyc.getIdtoSync(LocationSenorsAppDatabase.getAppDatabase(mContext));

                int sensorCount = GetLocationSensorsDataAsyc.getSensorsCount(LocationSenorsAppDatabase.getAppDatabase(mContext));
                ...
                Long sync_Time_diff =  System.currentTimeMillis() -  lastSyncTime.getLong(Constants.locationLastSync, System.currentTimeMillis());
                ...
                if ( (mtype==1) && (sync_Time_diff < Constants.One_Day) ) {
                    while (!jobCancelled && loopCount > 0) {
                        ...
                        lastsyncvalue = GetLocationSensorsDataAsyc.getIdtoSync(LocationSenorsAppDatabase.getAppDatabase(mContext));
                        loopCount--;
                    }
                }
                    ...
                    while (!jobCancelled &&  sensorCount>0 ) {
                        ...
                        lastsyncvalue=GetLocationSensorsDataAsyc.getIdtoSync(LocationSenorsAppDatabase.getAppDatabase(mContext));
                        sensorCount = GetLocationSensorsDataAsyc.getSensorsCount(LocationSenorsAppDatabase.getAppDatabase(mContext));
...
//                jobFinished(params, true);
    ...
    // added after room integration
    private void creatSensorsPost(final List<LocationSensor> sensor, Integer startindex, Integer endindex) {
        ...
        Call<LocationSensor> call = serverinterface.createLocationSensor("UTF-8",Constants.write_data,md5_value,sensor);
        // synchronous
        try {
            Response<LocationSensor> response = call.execute();
            ...
            if (response.code() == 200 ) {
                LocationSensor post = response.body();
/*
                String content = "";
...
*/
                ...
                lastSyncTimeEeditor.putLong(Constants.locationLastSync, (System.currentTimeMillis()) );
    ...
    */
    ...
    private void readSensorsPost(int type, Integer startindex, Integer endindex) {
        ...
        if (type == 0) {
            ...
            creatSensorsPost(GetLocationSensorsDataAsyc.syncSensorwithServer(LocationSenorsAppDatabase.getAppDatabase(mContext), startindex, endindex),startindex, endindex);
    ...
    private void deleteSensorsPosr(int type, Integer startindex, Integer endindex){
        ...
        if (type == 0) {
            ...
            GetLocationSensorsDataAsyc.DeleteSensorFromLocal(LocationSenorsAppDatabase.getAppDatabase(mContext), startindex, endindex);
...
// MD5 function
    ...
    private static String md5(List<LocationSensor> sensor) {
