...
//        }
                        ...
                        public void onComplete(@NonNull Task<Location> task) {
                            ...
                            if (task.isSuccessful() && task.getResult() != null) {
                                ...
                                if (mLocation==null)
                                {
                                    ...
                                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                                        myGlobalClass.startMyOwnLocationSetting(getString(R.string.Location_request_head),getString (R.string.Location_request_body));
                                    } else {
                                        myGlobalClass.startMyOwnLocationSetting_old(getString(R.string.Location_request_head),getString (R.string.Location_request_body));
                                    }
    ...
    private void onNewLocation(Location location) {
        ...
        // Use this method if new user ,TODO delete the old method
        if (sharedPref.getLong("patient_Time_Joined",System.currentTimeMillis())/1000 > Constants.impUpdateTime ){
            myGPSlocation = rotateGPSAroundPoint(location.getLatitude(),location.getLongitude(),location.getLatitude(), sharedPref.getInt(Constants.transformationid,360) );

            Log.d(TAG, "LATO: " +myGPSlocation.getLat()+ " LONO: " + myGPSlocation.getLon() + " trans:"+ sharedPref.getInt(Constants.transformationid,360) +
                    " LAT: "+ location.getLatitude() +" LON: "+ location.getLongitude() +" const time: "+Constants.impUpdateTime + " time joined:"+sharedPref.getLong("patient_Time_Joined",System.currentTimeMillis())/1000 );
        }
...
//        LocalBroadcastManager.getInstance(getApplicationContext()).sendBroadcast(intent);
    ...
    public LocationSensor rotateGPSAroundPoint(double lat, double lon, double h, double degree)
    {
        SharedPreferences sharedPref = PreferenceManager.getDefaultSharedPreferences(this);

        double latO = Double.parseDouble(sharedPref.getString (Constants.transformLatO,"-90"))  ;  // lat range in -90 to 90
        double lonO = Double.parseDouble(sharedPref.getString (Constants.transformLonO,"-180") ) ; // long range is -180 to 180

        double newLon =  lonO   + (Math.cos(Math.toRadians(degree)) * (lon - lonO) - Math.sin(Math.toRadians(degree))  * (lat - latO) / Math.abs(Math.cos(Math.toRadians(latO))));
        double newLat = latO + (Math.sin(Math.toRadians(degree)) * (lon - lonO) * Math.abs(Math.cos(Math.toRadians(latO))) + Math.cos(Math.toRadians(degree))   * (lat - latO));

        LocationSensor myGPSlocation = new LocationSensor();
        myGPSlocation.setAlt(h);
        myGPSlocation.setLon(newLon);
        myGPSlocation.setLat(newLat);
