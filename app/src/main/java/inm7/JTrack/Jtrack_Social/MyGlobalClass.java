...
import inm7.JTrack.Jtrack_Social.AccelerationSensor.AccelerationService;
import inm7.JTrack.Jtrack_Social.AccelerationSensor.SyncAccelerationClass;
import inm7.JTrack.Jtrack_Social.AccelerationSensor.SyncAccelerationJobService;
import inm7.JTrack.Jtrack_Social.ActiveLabeling.SyncActiveMonitoringJobService;
import inm7.JTrack.Jtrack_Social.ActiveLabeling.SyncActiveMonitoringStatClass;
import inm7.JTrack.Jtrack_Social.ActiveLabeling.UploadFiles;
import inm7.JTrack.Jtrack_Social.AppUsageStatSensor.AppUsageStatsJobService;
import inm7.JTrack.Jtrack_Social.AppUsageStatSensor.SyncAppUsageStatClass;
import inm7.JTrack.Jtrack_Social.AppUsageStatSensor.SyncAppUsageStatJobService;
import inm7.JTrack.Jtrack_Social.DetectedActivitySensor.BackgroundDetectedActivitiesService;
import inm7.JTrack.Jtrack_Social.DetectedActivitySensor.SyncActivityClass;
import inm7.JTrack.Jtrack_Social.DetectedActivitySensor.SyncActivityJobService;
import inm7.JTrack.Jtrack_Social.GyroscopeSensor.GyroscopeService;
import inm7.JTrack.Jtrack_Social.GyroscopeSensor.SyncGyroscopeClass;
import inm7.JTrack.Jtrack_Social.GyroscopeSensor.SyncGyroscopeJobService;
import inm7.JTrack.Jtrack_Social.LocationSensor.LocationUpdatesService;
import inm7.JTrack.Jtrack_Social.LocationSensor.SyncLocationClass;
import inm7.JTrack.Jtrack_Social.LocationSensor.SyncLocationJobService;
...
//start only if active labelling is selected (!=0)
    public void scheduleActiveLabelingSensorSycJob() {
       int isSensorSelectedPref =  PreferenceManager.getDefaultSharedPreferences(mContext).getInt("ActiveLabeling_switch",0);
...
// check if a service is running
    ...
    // to take user to enable location
    public void startMyOwnLocationSetting(String title, String body) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            Notification notification = notificationBuilder.setOngoing(false)
                    ...
                    .setPriority(NotificationManager.IMPORTANCE_HIGH)
                    .setCategory(Notification.CATEGORY_EVENT)
                    .setContentIntent(PendingIntent.getActivity(mContext.getApplicationContext(), 1, new Intent(Settings.ACTION_LOCATION_SOURCE_SETTINGS), PendingIntent.FLAG_ONE_SHOT))
                    .setAutoCancel(true)
                    .build();
            manager.notify(Constants.pushNotificationId, notification);
        }
    ...
    public void startMyOwnLocationSetting_old(String title, String body) {
    ...
    // to take user to enable location
    public void startAppNotRunningNotification(String title, String body) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            Notification notification = notificationBuilder.setOngoing(false)
                    ...
                    .setPriority(NotificationManager.IMPORTANCE_HIGH)
                    .setCategory(Notification.CATEGORY_EVENT)
                    .setContentIntent(PendingIntent.getActivity(mContext.getApplicationContext(), 1, new Intent(mContext, MainActivity.class), PendingIntent.FLAG_ONE_SHOT))
                    .setAutoCancel(true)
                    .build();
            manager.notify(Constants.pushNotificationId, notification);
        }
    ...
    public void startAppNotRunningNotification_old(String title, String body) {
        Notification mNotification;
    ...
    public boolean manual_syc(){
        ...
        UploadFiles up = new UploadFiles(mContext.getApplicationContext());
        up.getFiles();

        return LocationJobCanceled || ActivityJobCanceled || AppUsageJobCanceled || AccelerationJobCanceled || GyroscopeJobCanceled || AccelerationJobCanceled  || ActiveMonitoringJobCanceled;
            ...
            public void onResponse(Call<Patinet> call, Response<Patinet> response) {
                ...
                if (response.code() == 200) {
                   ...
                   stop_normal();
                    ...
                    startMyOwnForeground("Sie haben die Studie erfolgreich beendet","Sie können JuTrack Move jetzt deinstallieren.");
    ...
    public void checkIfLocationDisabled(){
        MyGlobalClass myGlobalClass = new MyGlobalClass(mContext);
        boolean isSensorSelectedPref = PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean("switch_preference_Location", false);
        if (isSensorSelectedPref != false) {


            Long askLocation_Time_diff =  System.currentTimeMillis() -  PreferenceManager.getDefaultSharedPreferences(mContext).getLong("location_notification_time", System.currentTimeMillis());
            Log.d("mainworker", "checkIfLocationDisabled: " + askLocation_Time_diff );


            if ( (askLocation_Time_diff>Constants.One_Day) ){

                LocationManager lm = (LocationManager) mContext.getSystemService(LOCATION_SERVICE);
                if (!lm.isProviderEnabled(LocationManager.GPS_PROVIDER) &&
                        !lm.isProviderEnabled(LocationManager.NETWORK_PROVIDER)) {

                    //update last time which notified user
                    SharedPreferences.Editor editor = PreferenceManager.getDefaultSharedPreferences(mContext).edit();
                    editor.putLong("location_notification_time", System.currentTimeMillis()).apply();

                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                        myGlobalClass.startMyOwnLocationSetting(mContext.getString(R.string.Location_request_head),mContext.getString (R.string.Location_request_body));
                    } else {
                        myGlobalClass.startMyOwnLocationSetting_old(mContext.getString(R.string.Location_request_head),mContext.getString (R.string.Location_request_body));
                    }
    ...
    public void checkIfappRuns(){
        ...
        Log.d("MainWorker", "doWork App status: background"+ isInBackground +" foreground: "+ isInForeground );

        // if not in foreground, not active labelling ==2, not main service running then notification and run it.
        if(  (!isInForeground) && (PreferenceManager.getDefaultSharedPreferences(mContext).getInt("ActiveLabeling_switch",0)!=2) && (!myGlobalClass.isMyServiceRunning(MainService.class))  ) {

            // also checked in Boot broadcast reciever

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                myGlobalClass.startAppNotRunningNotification(mContext.getString(R.string.alert_restartApp_title),mContext.getString (R.string.alert_restartApp_body));
            } else {
                myGlobalClass.startAppNotRunningNotification_old(mContext.getString(R.string.alert_restartApp_title),mContext.getString (R.string.alert_restartApp_body));
            }
