...
public class SyncActiveMonitoringStatClass {

    private Context mContext;
    private int mtype;
    public SyncActiveMonitoringStatClass(Context context, int type) {
        mContext = context;
        mtype= type;
    }
...
//
    ...
    private int patchsize = Constants.patchSizeXXSmall;
    ...
    public void syncOnBackGround() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
              ...
              */
                Integer lastsyncvalue=GetActiveLabelingSensorsDataAsyc.getIdtoSync(ActiveLabelingSenorsAppDatabase.getAppDatabase(mContext));

                int sensorCount = GetActiveLabelingSensorsDataAsyc.getSensorsCount(ActiveLabelingSenorsAppDatabase.getAppDatabase(mContext));
               ...
               Long sync_Time_diff =  System.currentTimeMillis() -  lastSyncTime.getLong(Constants.activeLabelingLastSync, System.currentTimeMillis());
               ...
               if ( (mtype==1) && (sync_Time_diff < Constants.One_Day) ) {
                   while (!jobCancelled &&  loopCount>0 ) {
                       ...
                       lastsyncvalue=GetActiveLabelingSensorsDataAsyc.getIdtoSync(ActiveLabelingSenorsAppDatabase.getAppDatabase(mContext));
                       loopCount--;
                   }
                   ...
                   while (!jobCancelled &&  sensorCount>0 ) {
                       ...
                       lastsyncvalue=GetActiveLabelingSensorsDataAsyc.getIdtoSync(ActiveLabelingSenorsAppDatabase.getAppDatabase(mContext));
                       sensorCount = GetActiveLabelingSensorsDataAsyc.getSensorsCount(ActiveLabelingSenorsAppDatabase.getAppDatabase(mContext));
    ...
    // added after room integration
    private void creatSensorsPost(final List<ActiveLabelingSensor> sensor, Integer startindex, Integer endindex) {
        String md5_value= md5(sensor).toString();
        SharedPreferences lastSyncTime=PreferenceManager.getDefaultSharedPreferences(mContext);
        SharedPreferences.Editor lastSyncTimeEeditor=lastSyncTime.edit();

        Call<ActiveLabelingSensor> call = serverinterface.createActiveLabelingSensor("UTF-8",Constants.write_data,md5_value,sensor);
        // synchronous
        try {
            Response<ActiveLabelingSensor> response = call.execute();
            ...
            if (response.code() == 200 ) {
                ActiveLabelingSensor post = response.body();
/*
                String content = "";
...
// put last sync  time
                ...
                lastSyncTimeEeditor.putLong(Constants.activeLabelingLastSync, (System.currentTimeMillis()) );
    ...
    */
    ...
    private void readSensorsPost(int type, Integer startindex, Integer endindex) {
        ...
        if (type == 0) {
            ...
            creatSensorsPost(GetActiveLabelingSensorsDataAsyc.syncSensorwithServer(ActiveLabelingSenorsAppDatabase.getAppDatabase(mContext), startindex, endindex),startindex,endindex);
    ...
    private void deleteSensorsPosr(int type, Integer startindex, Integer endindex){
        ...
        if (type == 0) {
            ...
            GetActiveLabelingSensorsDataAsyc.DeleteSensorFromLocal(ActiveLabelingSenorsAppDatabase.getAppDatabase(mContext), startindex, endindex);
    ...
    private static String md5(List<ActiveLabelingSensor> sensor) {
