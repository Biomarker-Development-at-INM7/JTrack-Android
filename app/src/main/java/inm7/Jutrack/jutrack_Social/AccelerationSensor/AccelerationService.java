...
public class AccelerationService extends Service implements SensorEventListener {
    ...
    List<AccelerationSensor> sensors = new ArrayList<>();
...
// movement detection part
    ...
    public void onDestroy() {
        ...
        Toast.makeText(this,getString(R.string.Acceleration_service_stopped) , Toast.LENGTH_SHORT).show();

        stopAccelerationSensor();
    ...
    public void onCreate() {
        ...
        Toast.makeText(this,getString(R.string.Acceleration_service_started) , Toast.LENGTH_SHORT).show();
...
//
    ...
    private void registerSensors() {
            ...
            sensormanager.registerListener(AccelerationService.this, sensormanager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER), freq,1000000);
    ...
    private void stopAccelerationSensor() {
    ...
    private void savetodb(  List<AccelerationSensor> sensors) {



        try {
            GetAccelerationSensorsDataAsyc.populateSensors(AccelerationSenorsAppDatabase.getAppDatabase(this), sensors);
        } catch (Exception e) {
    ...
    public void onSensorChanged(SensorEvent event) {
        if (event.sensor.getType()==Sensor.TYPE_ACCELEROMETER){
            ...
            sensor.setSensorname("accelerometer");
            sensor.setTimestamp(System.currentTimeMillis());
            sensor.setDeviceid(deviceid);
            sensor.setUsername(username);
            sensor.setStudyId(studyId);
            sensor.setX(event.values[0]);
            sensor.setY(event.values[1]);
            sensor.setZ(event.values[2]);
            steady_check(sensor);
            sensors.add(sensor);
...
//                        Log.d("SAVE",  Boolean.toString(to_save));
    ...
    public void steady_check(AccelerationSensor sensor){
